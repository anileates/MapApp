...
LocationSchema.pre("save", function (next) {
  ...
  if(this.location.coordinates.length !== 2) return next(new Error('Coordinates array must have 2 elements'));
  ...
  if(!(longitude >= -180 && longitude <= 180 && latitude >= -90 && latitude <= 90)) {
    return next (new Error('Longitude must be between -180 and 180, latitude must be between -90 and 90'))
  }
